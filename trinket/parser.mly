...
%token <int64> NUM
%token <string> ID
%token DEF ANDDEF ADD1 SUB1 LPARENSPACE LPARENNOSPACE RPAREN LBRACK RBRACK LCURLY RCURLY LET IN EQUAL COMMA PLUS MINUS TIMES IF COLON ELSECOLON EOF PRINT PRINTSTACK TRUE FALSE ISBOOL ISNUM ISTUPLE ISRECORD EQEQ LESSSPACE GREATER LESSEQ GREATEREQ AND OR NOT COLONEQ SEMI NIL LAMBDA BEGIN END SHADOW REC UNDERSCORE DOT LPPIPE RPPIPE
...
binop_operand :
  ...
  // Tables
  | LPPIPE exprs RPPIPE { ETable($2, full_span()) }
  // Simple cases
  | const { $1 }
  | id { $1 }
