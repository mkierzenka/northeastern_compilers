...
let input_file_test_suite () =
  let safe_readdir dir ext =
    ...
    with _ -> [] in
  "input-file-suite">:::[
      "do_pass"  >:::(List.map (fun f -> f>::test_does_run f)   (safe_readdir "input/do_pass"   ".diamond"));
      "do_err"   >:::(List.map (fun f -> f>::test_does_err f)   (safe_readdir "input/do_err"    ".diamond"));
      "dont_pass">:::(List.map (fun f -> f>::test_doesnt_run f) (safe_readdir "input/dont_pass" ".diamond"));
      "dont_err" >:::(List.map (fun f -> f>::test_doesnt_err f) (safe_readdir "input/dont_err"  ".diamond"))
    ]
