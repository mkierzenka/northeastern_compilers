...
let test_does_run filename test_ctxt =
  ...
  let runner = if Sys.file_exists valgrind then test_run_valgrind else test_run in
...
let test_does_err filename test_ctxt =
  ...
  let input = if Sys.file_exists infile then (string_of_file infile) else "" in
  test_err ~args:args ~std_input:input prog ("do_err/" ^ filename) err ~vg:(Sys.file_exists valgrind) test_ctxt
...
let test_doesnt_run filename test_ctxt =
  ...
  let args = if Sys.file_exists argsfile then String.split_on_char '\n' (chomp (string_of_file argsfile)) else [] in
  let input = if Sys.file_exists infile then (string_of_file infile) else "" in
  let runner = if Sys.file_exists valgrind then run_vg else run_no_vg in
...
let input_file_test_suite () =
  let safe_readdir dir ext =
    ...
    with _ -> [] in
  "input-file-suite">:::[
      "do_pass"  >:::(List.map (fun f -> f>::test_does_run f)   (safe_readdir "input/do_pass"   ".cobra"));
      "do_err"   >:::(List.map (fun f -> f>::test_does_err f)   (safe_readdir "input/do_err"    ".cobra"));
      "dont_pass">:::(List.map (fun f -> f>::test_doesnt_run f) (safe_readdir "input/dont_pass" ".cobra"));
      "dont_err" >:::(List.map (fun f -> f>::test_doesnt_err f) (safe_readdir "input/dont_err"  ".cobra"))
    ]
